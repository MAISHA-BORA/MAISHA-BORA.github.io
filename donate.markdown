...
document.addEventListener('DOMContentLoaded', function() {
    // Configuration
    const BASE_DOMAIN = window.location.origin; // Use current domain
    const API_BASE = `${BASE_DOMAIN}/api`; // Your backend API
    ...
    const paymentMethods = {
        'mpesa': { name: 'M-Pesa', type: 'mobile' },
        'airtel': { name: 'Airtel Money', type: 'mobile' },
        'tigo': { name: 'Tigo Pesa', type: 'mobile' },
        'visa': { name: 'Visa Card', type: 'card' },
        'mastercard': { name: 'MasterCard', type: 'card' },
        'selcom': { name: 'Selcom Pesa', type: 'selcompesa' }
    };
    ...
    function updatePaymentMethod() {
        ...
        // Update currency options based on payment method type
        if (config.type === 'card') {
            // Card payments can use TZS or USD
            updateCurrencyOptions(['TZS', 'USD', 'EUR', 'GBP']);
        } else {
            // Mobile payments only TZS
            updateCurrencyOptions(['TZS']);
        }
        
        // Update phone requirement
        const phoneField = document.getElementById('donorPhone');
        phoneField.required = config.type !== 'card';
        
        if (config.type !== 'card') {
            phoneField.pattern = '^255[67]\\d{8}$';
    ...
    function updateAmountDisplay() {
        // For display purposes, show in selected currency
    ...
    async function handleDonationSubmit(e) {
        ...
        try {
            ...
            const donationData = {
                ...
                walletType: paymentMethodSelect.value // For mobile payments
            };
            
            console.log('Sending donation request:', donationData);
            
            // Call YOUR backend API (create-order.js)
            ...
            showError(`Payment Error: ${error.message || 'Please try again later'}`);
    ...
    function validateForm() {
        ...
        if (config && config.type !== 'card') {
            // Mobile payments require valid Tanzanian phone
            const phone = phoneField.value.trim();
            const phoneRegex = /^255[67]\d{8}$/;
            
            if (!phone) {
                showFieldError(phoneField, 'Phone number is required for mobile payments');
                isValid = false;
            } else if (!phoneRegex.test(phone)) {
                showFieldError(phoneField, 'Please enter a valid Tanzanian phone number (format: 2557XXXXXXX)');
                isValid = false;
            }
        }
                ...
                <div class="modal-body" style="text-align: center;">
                    <div style="margin-bottom: 25px;">
                        ...
                        <p style="color: var(--text-light); font-size: 0.95rem;">
                            Reference: <strong>${paymentResult.reference || paymentResult.transactionId}</strong>
                        </p>
                    </div>
                    ...
                    ">
                        <h4 style="margin-top: 0; color: var(--primary);">Instructions:</h4>
                        <ol style="margin: 15px 0; padding-left: 20px;">
                            <li style="margin-bottom: 10px;">Open your mobile money app</li>
                            <li style="margin-bottom: 10px;">Tap "Scan QR Code" or use Send Money</li>
            ...
            <div class="modal-content" style="max-width: 450px;">
                ...
                <div class="modal-body" style="text-align: center;">
                    <div style="margin-bottom: 30px;">
                        ...
                        <h3 style="color: var(--primary); margin-bottom: 15px;">Payment Request Sent</h3>
                    ...
                    ">
                        <p style="margin: 0; font-size: 0.9rem; color: #2e7d32;">
                            ...
                            <code style="
                                ...
                                margin-left: 10px;
                                color: var(--dark);
                            ">${paymentResult.transactionId}</code>
                        </p>
                    </div>
